function [S, BL] = calcSpect(S, BL, fRange, Fs, timeWindow)

% Handle input arguments - if not entered
if (nargin < 5) timeWindow = 20;    end % ms
if (nargin < 4) Fs         = 3000;  end % Hz
if (nargin < 3) fRange     = 1:600; end % Hz

% Initialize spect structure
...
S.spect.power = zeros(length(fTemp), length(tTemp), ev);

% Spectral Analysis of valid tSeries:
ev = 1;
for i = 1:length(tSeries)
  if ~isempty(tSeries{i}) && (length(tSeries{i}) == spectSamples)
    [~, S.spect.fRange, S.spect.tRange, S.spect.power(:,:,ev)] = spectrogram(tSeries{i}, nWin, nOv, fRange, Fs, 'yaxis');
    
    % Normalize to spectrogram unless baseline supplied:
    if isempty(BL)
      pAve = mean(S.spect.power, 2);
      pStd = std(S.spect.power, 0, 2);
...
% Calculate average spectrogram if more than one tSeries:
if length(tSeries) > 1
  S.spect.powerAve = mean(S.spect.power, 3);
