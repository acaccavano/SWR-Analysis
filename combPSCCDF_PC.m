function S = combPSCCDF_PC(param, dataFolders)
%% S = combCaCorr(param, dataFolderCT, dataFolderAD, saveFile)
%
...
maxAmp = max([max(S(1).ampSpontAll) max(S(1).ampSWRAll) max(S(2).ampSpontAll) max(S(2).ampSWRAll) max(S(3).ampSpontAll) max(S(3).ampSWRAll) max(S(4).ampSpontAll) max(S(4).ampSWRAll)]);
maxArea = max([max(S(1).areaSpontAll) max(S(1).areaSWRAll) max(S(2).areaSpontAll) max(S(2).areaSWRAll) max(S(3).areaSpontAll) max(S(3).areaSWRAll) max(S(4).areaSpontAll) max(S(4).areaSWRAll)]);
